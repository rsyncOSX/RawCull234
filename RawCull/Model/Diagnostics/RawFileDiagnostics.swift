...
nonisolated enum RawFileDiagnostics {
    static func log(for file: FileItem) -> String {
        ...
        logger.line("captureDate: \(file.captureDate.map(formattedDate) ?? "nil")")
        logger.line("captureTimeZoneOffsetSeconds: \(string(file.captureTimeZoneOffsetSeconds))")
        logger.line("captureTimeSource: \(file.usesFileModificationDateForCaptureTime ? "file modification date" : "EXIF")")
    ...
    private static func logExif(_ exif: ExifMetadata?, to logger: inout RawFileDiagnosticLogger) {
        ...
        logger.line("exposureTimeSeconds: \(string(exif.exposureTimeSeconds))")
        ...
        logger.line("focalLengthMM: \(string(exif.focalLengthMM))")
        logger.line("exposureCompensationEV: \(string(exif.exposureCompensationEV))")
